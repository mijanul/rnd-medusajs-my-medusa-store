...
export default async function handlePriceSync({
  ...
  const PRODUCT_MODULE = "productService";
  ...
  try {
    ...
    console.log("🔄 Price sync triggered:", data);

    // Get the product variant ID from the event
    ...
    // Convert from minor units (e.g., 220000) to major units (2200.00)
